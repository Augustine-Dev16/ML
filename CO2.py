...
#transform model parameters back to original
...
#retrain using engine size only
X_train_1 = X_train[:,0]
...
#scatter plot CO2 EMISSIONS vs Engine size
plt.scatter(X_train_1, y_train,  color='blue')
plt.plot(X_train_1, coef_1[0] * X_train_1 + intercept_1, '-r')
plt.xlabel("Engine size")
plt.ylabel("Emission")

#scatter plot COE EMISSIONA vs Engine size (test data)
X_test_1 = X_test[:,0]
...
#retrain using fuel consumption only
X_train_2 = X_train[:,1]
...
#scatterplot CO2 EISSIONS vs Engine size (test data)
X_test_2 = X_test[:,1]
